...
- [x] Golden-file test set of real anonymised emails with expected
      ...
      `docs/LEARNED.md`, 2026-09-14). **37/41 passing.** The 16
      edge-case fixtures found 2 real, fixed bugs (see below) plus 2
      real, not-yet-fixed correctness gaps (payment-failure emails
      misread as successful renewals; a subscription pause forced into
      `renewal` with a hallucinated date — both stem from the same root
      cause: no signal type exists for "not actually a billing event")
      and reconfirmed the known `new`/`trial_conversion` boundary
      weakness. `pnpm test:golden` run against the real Claude API.
