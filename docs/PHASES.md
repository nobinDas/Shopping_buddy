...
- [x] Golden-file test set of real anonymised emails with expected
      ...
      `docs/LEARNED.md`, 2026-09-14). **39/41 passing.** The 16
      edge-case fixtures found and led to fixing 3 real bugs (see below)
      and reconfirmed the known `new`/`trial_conversion` boundary
      weakness (`hulu-trial-started`, `netflix-extra-member-addon` — 2
      remaining failures, a separate, still-open issue). `pnpm
      test:golden` run against the real Claude API.
