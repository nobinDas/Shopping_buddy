...
import { getAllLists, getItemById, insertItem, deleteItemRow } from '@/server/db/queries/shopping';
  ...
  it('attaches each list its own items only', async () => {
    await expect(
      db.transaction(async (tx) => {
        const [listA] = await tx.insert(shoppingLists).values(buildShoppingList({ name: 'A' })).returning();
        const [listB] = await tx.insert(shoppingLists).values(buildShoppingList({ name: 'B' })).returning();
