...
import type { PriceCheckResult } from '@/server/services/shopping.service';
import {
  addItemAction,
  updateItemAction,
  deleteItemAction,
  toggleItemCheckedAction,
  checkItemPriceAction,
} from '@/app/(dashboard)/shopping/actions';

function listSubtotal(items: ShoppingItemRow[]): Money | null {
  const [first, ...rest] = items.filter(
    (item): item is ShoppingItemRow & { unitPriceMinor: number; currency: string } =>
      item.unitPriceMinor !== null && !item.checked,
  );
...
export function ShoppingLists({
  ...
  function checkPrice(id: string) {
    ...
    startTransition(() => {
      void (async () => {
        const result = await checkItemPriceAction(id);
  ...
  return (
    <Tabs value={activeListId} onValueChange={setActiveListId}>
      ...
      {lists.map((list) => {
        const total = listSubtotal(list.items);
        const unpricedCount = list.items.filter(
          (item) => item.unitPriceMinor === null && !item.checked,
        ).length;
        ...
        return (
          <TabsContent key={list.id} value={list.id}>
                ...
                return (
                  <div key={item.id} className="border-b border-rule py-3.5">
                    <div className="flex items-start gap-2.5">
                      ...
                      <button
                        type="button"
                        aria-label={`Check ${item.name}'s price on Walmart`}
                        onClick={() => {
                          checkPrice(item.id);
                        }}
                        disabled={priceCheck?.status === 'checking'}
                        className="flex-none p-0.5 text-ink-muted hover:text-ink disabled:opacity-40"
                      >
                    ...
                    {priceCheck && (
                      <p className="mt-1.5 pl-[26px] text-[11px] text-ink-muted">
                        {priceCheck.status === 'checking' && 'Checking Walmart…'}
                        {priceCheck.status === 'found' &&
                          `Updated from Walmart${priceCheck.item.lastPriceCheckedAt ? ` · ${formatDate(new Date(priceCheck.item.lastPriceCheckedAt).toISOString().slice(0, 10))}` : ''}.`}
                        {priceCheck.status === 'not_found' && 'No match found on Walmart.'}
                        {priceCheck.status === 'error' && (
                          <span className="text-flag">{priceCheck.message}</span>
                        )}
                      </p>
                    )}
            ...
            <p className="pt-2 pl-[26px] text-[11px] leading-relaxed text-ink-muted">
              New items land with quantity 1 and no store set. Use the pencil to set details, the
              magnifier to check its Walmart price.
            </p>
